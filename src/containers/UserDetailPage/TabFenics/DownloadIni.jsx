...
function makeBlob(state: StateType): Blob {
  ...
  const onlyPCFenicsData = state.data.fenics.filter(
    (item: FenicsType) => item.is_mobile === 0
  );
  const selectionData = selection.map(num => ({
    update_flag: 'A',
    prefix: 'hopecl-',
    user_label: onlyPCFenicsData[num].fenics_id,
    init_password: onlyPCFenicsData[num].password,
    id_group: 'お客様　システム１',
    access_control_group: 'お客様用　システム１',
    start_date:
      (onlyPCFenicsData[num].start_on &&
        moment(onlyPCFenicsData[num].start_on).format(
          'YYYYMMDD'
        )) ||
      '',
    end_date: '',
